...
names(exam_dat)=c('LIHC','COAD','KIRC', 'CESC','LUAD')
...
cescids=as.numeric(na.omit(match(cescids, cesc.met$merged.dat[,1])))
...
exam_dat$COAD$met=as.matrix(coad.met$merged.dat)[coadids,]
rm(coad.met)
...
exam_dat$KIRC$met=as.matrix(kirc.met$merged.dat)[kircids,]
rm(kirc.met)

lihcids=rownames(lihc.met$clinical)[which((lihc.met$clinical[,grep('race',colnames(lihc.met$clinical))]=='white'|lihc.met$clinical[,grep('race',colnames(lihc.met$clinical))]=='black or african american')&lihc.met$clinical[,which(colnames(lihc.met$clinical)=='gender')]=='female')]
lihcids=as.numeric(na.omit(match(lihcids, lihc.met$merged.dat[,1])))
exam_dat$LIHC$met=as.matrix(lihc.met$merged.dat)[lihcids,]
rm(lihc.met)
...
luadids=as.numeric(na.omit(match(luadids, luad.met$merged.dat[,1])))
